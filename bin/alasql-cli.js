...
let alasqln = require('alasqln');
let path = require('path');
let fs = require('fs');
let stdin = process.openStdin();
let yargs = require('yargs')
	.strict()
	.usage('AlaSQL command-line utility (version ' + alasqln.version + ')\n\nUsage: $0 [options] [sql] [params]')
...
.version('v', 'Echo AlaSQL version', alasqln.version)
	.alias('v', 'version')
...
.epilog('\nMore information about the library: www.alasqln.org')
...
function execute(sql, params) {
	...
	alasqln.promise(sql, params)
		.then(function (res) {
			if (!alasqln.options.stdout) {
